...
class Authentication extends Component {
    ...
    handleChange = event => {
        ...
        if (input.name === 'email')
            ...
            this.setState({ lastname: input.value, verifyLast: /^([a-z]+(( |')[a-z]+)*)+([-]([a-z]+(( |')[a-z]+)*)+)*$/iu.test(input.value) === false ? 'is-invalid' : 'is-valid' })
        else if (input.name === 'firstname')
            this.setState({ firstname: input.value, verifyFirst: /^([a-z]+(( |')[a-z]+)*)+([-]([a-z]+(( |')[a-z]+)*)+)*$/iu.test(input.value) === false ? 'is-invalid' : 'is-valid' })
